...
namespace PhisilyncFinal.Services
{
    public class LocalDb
    {
        ...
        public List<Event> GetCurrentTreatmentByID()
        {
           
            
            
            var todayPortion = DateTime.Now.Date;
            var allEvents = _dbConnection.Table<Event>().ToList();
            var list =allEvents.Where(allEvents=>allEvents.EventDate.Date==todayPortion).ToList();
            return list;
            
           // var events = _dbConnection.Table<Event>().ToList();

            
        }

        public List<Event> GetTodayTreatment()
        {
            return _dbConnection.Table<Event>().Where(x => x.EventDate == DateTime.Today).ToList();
        }

        public Event GetTodayRelease()
        {
            return _dbConnection.Table<Event>().Where(x => x.EventDate == DateTime.Today && x.Category == "Release").FirstOrDefault();
        }
